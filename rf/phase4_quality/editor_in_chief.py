...
class EditorInChief(BaseAgent):
    ...
    async def run(self, context: dict[str, Any]) -> dict[str, Any]:
        user_msg = (
            ...
            f"### Hardware Audit\n{_safe_dump(context.get('hardware_audit', {}), 7500)}\n\n"
            f"### Novelty Audit\n{_safe_dump(context.get('novelty_audit', {}), 7500)}\n\n"
            f"### Source Analysis\n{_safe_dump(context.get('source_analysis', {}), 10000)}\n\n"
            f"### Math Translation\n{_safe_dump(context.get('math_translation', {}), 10000)}\n\n"
            f"### Conflict Report\n{_safe_dump(context.get('conflict_report', {}), 7500)}\n\n"
            f"### Final Experiment Plan\n{_safe_dump(context.get('final_plan', {}))}\n\n"
            f"### Quality Report\n{_safe_dump(context.get('quality_report', {}), 7500)}\n\n"
            f"### Publication Strategy\n{_safe_dump(context.get('publication_strategy', {}), 7500)}\n\n"
